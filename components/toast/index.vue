<template>
  ...
  >
    ...
    >
      <div class="v-toast-icon" v-if="type !== 'text'">
        <slot name="icon">
          <Loading v-if="type === 'loading'" />
          <i
            v-else
            class="v-toast-icon-image"
            :class="'v-toast-icon-' + type"
          />
        </slot>
      </div>
      <div class="v-toast-title">
        <slot>{{ title }}</slot>
      </div>
    </div>
  </Delay>
