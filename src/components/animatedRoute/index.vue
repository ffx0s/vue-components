...
export default {
    ...
    if (!installed) {
      ...
      Vue.mixin({
        beforeRouteEnter(to, from, next) {
          ...
          next(vm => {
            // 进入当前页面的路由时，滚动到之前的位置
            scroller.scrollTo(vm, to)
          })
        },
        beforeRouteLeave(to, from, next) {
          // 离开当前页面的路由时，保存页面滚动位置
          scroller.saveScrollTop(this, from)
          next()
        }
      })
  ...
  methods: {
    ...
    afterEnter(el) {
      this.$animatedRoute.$emit('afterEnter', el)
    },
    afterLeave(el) {
      this.$animatedRoute.$emit('afterLeave', el)
    },
    enterCancelled(el) {
      this.$animatedRoute.$emit('enterCancelled', el)
    },
    // leaveCancelled 只用于 v-show 中
    leaveCancelled(el) {
      this.$animatedRoute.$emit('leaveCancelled', el)
    }
  }
}
</script>
